...
export default function Page() {
  return (
    <main className="min-h-screen bg-gradient-to-br from-red-950 via-black to-red-900 text-white">
      ...
      <section id="home" className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          <Image
            ...
            priority
          />
          <div className="absolute inset-0 bg-gradient-to-br from-red-950/80 via-black/70 to-red-900/80"></div>
        </div>
        
        <div className="relative z-10 text-center px-4">
          <div className="animate-fade-in-up">
            <Badge className="bg-red-600 hover:bg-red-700 text-white border-0 mb-8 text-sm font-medium px-4 py-2 animate-bounce">
              June 15-17, 2025
            </Badge>
            <h1 className="text-6xl md:text-8xl font-black tracking-tight mb-8 gradient-text-red drop-shadow-2xl">
              Elite Car Show
            </h1>
            <p className="text-xl md:text-2xl text-red-200 mb-12 max-w-2xl mx-auto font-light drop-shadow-lg">
              Where luxury meets innovation
            </p>
            <div className="flex justify-center">
              <button className="bg-red-600 hover:bg-red-700 text-white px-8 py-4 text-lg font-medium transition-all duration-300 transform hover:scale-105 hover:shadow-2xl hover:shadow-red-500/50">
                Get Tickets
              </button>
            </div>
          </div>
      ...
      {/* Event Info */}
      <section id="info" className="py-24 bg-gradient-to-br from-red-950 to-black text-white">
        <div className="container mx-auto px-4">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div className="space-y-8 animate-slide-in-left">
              <h2 className="text-5xl font-black mb-8">Event Details</h2>
              <div className="space-y-6">
                <div className="flex items-center gap-4 p-4 bg-red-900/30 rounded-lg border border-red-500/20 hover:border-red-500/40 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-red-500/25">
                  <MapPin className="w-6 h-6 text-red-400" />
                  <div>
                    <div className="font-medium text-lg">Los Angeles Convention Center</div>
                    <div className="text-red-300">1201 S Figueroa St, Los Angeles, CA</div>
                  </div>
                </div>
                <div className="flex items-center gap-4 p-4 bg-red-900/30 rounded-lg border border-red-500/20 hover:border-red-500/40 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-red-500/25">
                  ...
                  <div>
                    ...
                    <div className="text-red-300">+1 (213) 555-0123</div>
                  </div>
                </div>
              </div>
              
              <div className="flex gap-4">
                <a href="https://instagram.com" className="p-3 bg-red-600/20 rounded-lg hover:bg-red-600/30 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-red-500/25">
                  <Instagram className="w-6 h-6" />
                </a>
                <a href="https://facebook.com" className="p-3 bg-red-600/20 rounded-lg hover:bg-red-600/30 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-red-500/25">
                  <Facebook className="w-6 h-6" />
                </a>
                <a href="https://twitter.com" className="p-3 bg-red-600/20 rounded-lg hover:bg-red-600/30 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-red-500/25">
...
function Carousel({ cars }: { cars: Array<{
  ...
  return (
    <div className="relative w-full h-full">
            ...
            <div className="relative h-full w-full">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-12 text-white">
                <div className={`transition-all duration-1000 delay-300 ${
                  ...
                  <Badge className="bg-red-600 text-white border-0 mb-6 text-sm animate-pulse">
                    {car.category}
                  </Badge>
                  <h3 className="text-5xl md:text-7xl font-black mb-4 text-shadow-lg">{car.name}</h3>
                  <p className="text-2xl md:text-3xl text-red-200 mb-6 font-light">{car.brand}</p>
                  <div className="text-3xl font-bold mb-8">{car.price}</div>
                  
                  {/* Car Specs */}
                  <div className="grid grid-cols-3 gap-6 max-w-md">
                    <div className="text-center p-4 bg-red-900/50 rounded-lg backdrop-blur-sm border border-red-500/30 hover:border-red-500/60 transition-all duration-300 hover:scale-105">
                  ...
                  <a 
                    href={`/car/${car.id}`}
                    className="inline-block mt-8 bg-red-600 hover:bg-red-700 text-white py-4 px-8 rounded-lg text-lg font-medium transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-red-500/50"
                  >
                    View Details
                  </a>
                </div>
      ...
      <button
        onClick={prevSlide}
        className="absolute left-8 top-1/2 -translate-y-1/2 bg-black/40 backdrop-blur-sm text-white p-4 rounded-full hover:bg-black/60 transition-all duration-300 transform hover:scale-110 hover:shadow-2xl hover:shadow-red-500/50 z-10"
      >
        <ChevronLeft className="w-8 h-8" />
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-8 top-1/2 -translate-y-1/2 bg-black/40 backdrop-blur-sm text-white p-4 rounded-full hover:bg-black/60 transition-all duration-300 transform hover:scale-110 hover:shadow-2xl hover:shadow-red-500/50 z-10"
      >
        ...
        {cars.map((_, index) => (
          <button
            ...
            className={`w-4 h-4 rounded-full transition-all duration-300 ${
              index === currentSlide ? 'bg-red-500 scale-125 shadow-lg shadow-red-500/50' : 'bg-white/50 hover:bg-white/80'
            }`}
          />
        ))}
      </div>
    ...
    <header className="fixed top-0 z-50 w-full bg-black/80 backdrop-blur-md border-b border-red-500/20">
      <div className="container mx-auto flex h-20 items-center justify-between px-4">
        <a href="#" onClick={() => scrollToSection('home')} className="flex items-center gap-3 font-bold text-xl hover:scale-105 transition-transform duration-300 cursor-pointer">
          <div className="p-2 bg-red-600 rounded-lg animate-glow">
        ...
        <nav>
          <ul className="flex items-center gap-8 text-red-200">
